...
namespace NI.Data.Storage.Tests.Model {
	...
	public class RelationshipTests {
		...
		public void EqualsAndHashCode() {
			...
			var rel1 = new Relationship(class1, pred1, class2, false,false);
			var rel1same = new Relationship(class1, pred1, class2, false,false);
			Assert.AreEqual(
				rel1same, rel1);
			Assert.AreEqual(
				rel1same.GetHashCode(), rel1.GetHashCode() );
			Assert.AreNotEqual(
				new Relationship(class2, pred1, class1, false, false), rel1);
			Assert.AreNotEqual(
				new Relationship(class1, pred2, class2, false, false), rel1);

			var rel2 = new Relationship(class2, pred2, class1, false, false);
